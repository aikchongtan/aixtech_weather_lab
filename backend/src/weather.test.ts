...
describe('SingaporeWeatherClient current conditions', () => {
  it('uses the nearest station with a valid reading for each current condition', async () => {
    vi.stubGlobal('fetch', async (input: RequestInfo | URL) => {
      ...
      if (url.includes('wind-speed')) return jsonResponse(readingPayload(4.5));
      if (url.includes('wind-direction')) return jsonResponse(readingPayload(156));
  ...
  it('keeps wind speed when the direction source fails', async () => {
    vi.stubGlobal('fetch', async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.includes('two-hr-forecast')) return jsonResponse(forecastPayload);
      if (url.includes('air-temperature')) return jsonResponse(readingPayload(30.4));
      ...
      if (url.includes('4-day-weather-forecast')) return jsonResponse(fourDayPayload);
      if (url.includes('wind-speed')) return jsonResponse(readingPayload(4.5));
      if (url.includes('wind-direction')) throw new Error('direction unavailable');
      throw new Error(`Unexpected request: ${url}`);
    });
    ...
    expect(weather).toMatchObject({ wind_speed_knots: 4.5, wind_direction_degrees: null });
  });

  it('keeps wind direction when the speed source fails', async () => {
    vi.stubGlobal('fetch', async (input: RequestInfo | URL) => {
      const url = String(input);
      if (url.includes('two-hr-forecast')) return jsonResponse(forecastPayload);
      if (url.includes('air-temperature')) return jsonResponse(readingPayload(30.4));
      ...
      if (url.includes('24-hour-weather-forecast')) return jsonResponse(twentyFourHourPayload);
      if (url.includes('4-day-weather-forecast')) return jsonResponse(fourDayPayload);
      if (url.includes('wind-speed')) throw new Error('speed unavailable');
      if (url.includes('wind-direction')) return jsonResponse(readingPayload(156));
      throw new Error(`Unexpected request: ${url}`);
    });
    ...
    expect(weather).toMatchObject({ wind_speed_knots: null, wind_direction_degrees: 156 });
  });
});
