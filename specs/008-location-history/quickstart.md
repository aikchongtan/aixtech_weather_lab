...
4. Preserve all dashboard workflows and implement the atomic SQLite write boundary before wiring the detail UI. Verify direct `/locations/:id` navigation in development and compiled production; if SPA fallback is needed, it must serve only non-`/api/*` navigation requests.
